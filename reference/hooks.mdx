...
Fetches and manages the list of available Quran reciters from the MP3 Quran API.

**Location:** `src/hooks/use-reciters.ts:11`

**Parameters:** None (uses locale from react-intl context)

<ResponseField name="reciters" type="Reciter[]">
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      {reciters.map(reciter => (
        <button
          key={reciter.id}
          onClick={() => setSelectedId(reciter.id)}
          className={selectedId === reciter.id ? 'bg-blue-500' : 'bg-gray-200'}
        >
          <h3>{reciter.name}</h3>
          <p>{reciter.moshaf.name}</p>
        </button>
      ))}
    </div>
  );
...
Listens for the Escape key and triggers a callback when pressed.

**Location:** `src/hooks/use-escape-key-hook.ts:6`

<ParamField path="handleClose" type="() => void" required>
